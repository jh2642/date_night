...
app.get('/api/v1/search', function (request, response) {
    https.get('https://maps.googleapis.com/maps/api/place/textsearch/json?query=' + request.query.query + '&location=' + request.query.location + '&radius=16094' + '&key=' + process.env.GOOGLE_PLACES_KEY,
...
app.post('/events/datenight', function (request, response) {
    ...
    .then(function(users) {
        if(users[0].date_email === '' || users[0].date_email === null) {
            knex('users')
            .where('id', request.body.user_id)
            .update({date_name: request.body.date_name, date_email: request.body.date_email, guest1: request.body.guest1Email,guest2: request.body.guest2Email,guest3: request.body.guest3Email,guest4: request.body.guest4Email,guest5: request.body.guest5Email, your_email: request.body.yourEmail})
            .then(function(){
...
app.get('/events/datenight/datee', function (request, response) {
    knex('events')
    .select()
    .where('date_email', request.query.yourEmailAddress)
    .then(function(details) {
        response.json(details)
    })
})
